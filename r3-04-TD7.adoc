...
:tdnum: TD7 - Modèle MVC par l'exemple : `JTable`
...
A noter que Java Swing et JavaFX sont deux bibliothèques graphiques, JavaFX plus récente que Swing.
Elles comportent des composants "assez proches", plus riches pour java FX.
Nous allons ici étudier JavaFX et les aspects composants graphiques vous seront donnés en TD/TP.
...
.Version corrigée
=====
Faire la démo de jtable5 => objectif final du TP
=====
endif::prof[]
...
Avec ce que nous savons de Swing ou Java FX, quels composants graphiques utiliserions nous pour programmer le tableau ci-dessus ?
...
====
En JavaFX :
.	Un GridPane 4 colonnes et x lignes suivant le nombre de lignes
..	Ou un BorderPane et plusieurs VBox
.	Première ligne : des Label
.	Lignes suivantes : un TextField par cellule
En Swing :
.	Un JPanel
...
..	Or tout le tableau n'est pas visible, maximum 50 lignes => maximum 400 JTextField utiles
...
Côté graphique : un JTable qui a une méthode *`setModel()`* pour associer un modèle.
...
*	`public Object getValueAt(int row, int column)` : l’objet à afficher dans la ligne et la colonne indiquées (sa méthode toString est utilisée).
...
class SimpleTableModel extends AbstractTableModel {
	...
	public Object getValueAt(int row, int column) {
		return new Integer((1+row)*(1+column));
	}
}
...
*	redéfinit aussi `public String getColumnName(int column)` qui donne les en-têtes de colonne (sinon "A", "B", ... par défaut).
...
// On suppose disposer dans JTable :
// - d'un attribut this.dataModel pour interroger le datamodel
// - d'une méthode this.maxViewableCols() : nb max de colonnes affichables
// - d'une méthode this.maxViewableRows() : nb de lignes affichables
// - d'une méthode this.drawAtScreen(x) : dessine une valeur sur la fenêtre (la position se calcule "toute seule")
.
.
.
.
...
*Question* : En supposant que le table ci-dessus reçoive le message paint() pour s'afficher, compléter le diagramme de séquence suivant avec les messages échangés entre table et dataModel pour réaliser l'affichage en suivant l'algorithme défini.
...
*	`boolean isCellEditable(int row, int column)` : renvoie true si la cellule peut être modifiée (par défaut false)
*	`void setValueAt(Object val, int row, int column)` : modifie une donnée du modèle (par défaut aucune modification)
