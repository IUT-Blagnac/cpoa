...
:authorDefault:  Dut/Info-S3/R3.03
:tdnum: TD 3
...
Dans cet exercice, nous allons voir qu'il est utile d'utiliser une combinaison de modèles (ici classe et séquence) pour décrire des situations complexes.
Nous allons aussi en profiter pour explorer comment on peut maintenir la cohérence entre plusieurs objets qui sont liés sans pour autant maintenir d'association fortes entre ces objets.
...
un diagramme de séquence illustrant l'utilisation du patron _Observer_. Commencez par l'appel d'une méthode _setState()_ sur l'observé (le sujet).
====
...
. L'un est une interface, l'autre une classe. Serez-vous retrouver la nature de Observer et Observable ? Pourquoi ?
...
- ...
. Dans un attribut de type Array, liste chaînée (ArrayList), ...
. Le sujet peut s'identifier auprès des observateurs via l'interface `update`
. Par exemple :
...
. En Java, seul l'objet lui-même peut "initier" une notification à ses observateurs grâce à une variable d'état à visibilité "protected".
Il faudra s'assurer au sein de la classe concrête Observable que la séquence est respectée sous peine de ne pas avoir de notification. 
. Par exemple :
- Push Model => Beaucoup
- Pull Model => Le moins possible
. On parle alors de _publish-subscribe_
. Oui ! Il héritera de Observable et implémentera Observer.
. Par exemple :
...
[WARNING]
====
Que doit-on rajouter pour adapter le diagramme de classe fait précédement à l'implémentation Java de Observer/Observable ?
====
...
.Solution
========
- appeler la méthode _hasChanged()_ dans _setState()_
- + renommer _notify_ en _notifyObserver_
========
endif::prof[]
...
- Faire le lien avec les listeners vus en IHM l'an dernier.
====
//----------------------------------------------------- Correction -------------------------
ifdef::prof[]
[CAUTION]
