...
= {doctitle}
:Author:  {authorDefault} - (Semaine 45)
:lang: fr
...
//----------- définitions --------------
:sitecours: http://iut-blagnac.github.io/cpoa/[site du cours]
:pre: PreReq
:objtd: ObjTD
:duree: Durée
:depot: À rendre
...
| {duree}	| *1* TD et *2* TP de 1,5h (sur 2 semaines différentes).
|======================
...
n'est requise. Le cours du semestre 2 (R2.01 Dév Objet et R2.03 Qualité) est considéré comme acquis!

TIP: Mais pour les prochains, n'hésitez pas à (re)lire régulièrement le cours.
...
[options=interactive]
- [ ] Le même code pourrait être dupliqué (réécrit) entre les sous-classes.
...
.Solution
====
- [*] Le même code pourrait être dupliqué (réécrit) entre les sous-classes : ex. si plusieurs canards ne volent pas comme le canard en plastique sans en hériter
- [*] Les changements de méthodes de comportements au moment de l'exécution sont difficiles : code défini dans la classe pour toutes les instances.
- [ ] Nous ne pouvons pas avoir de canards qui dansent.
- [*] Il est difficile de connaître tous les comportements des canards (à l'avance)
- [ ] Les canards ne peuvent pas voler et cancaner en même temps.
- [*] Les modifications peuvent affecter involontairement d'autres canards : par héritage
====
endif::prof[]
//----------------------------------------------------- fin Correction -------------------------
...
Vous êtes confrontés au même problème "classique" que l'on retrouve souvent : *LE CHANGEMENT*!
...
WARNING: Ce mots-croisés est tiré du livre cité en entête, et il comporte donc
