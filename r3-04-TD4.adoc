...
4. J'ai compris ce qu'est un patron et j'ai grand soif d'en apprendre d'autres que _Strategy_
...
Voici la classe contrôleur du bouilleur industriel de _Bonchoco, SA_. On suppose que le programme contrôle *une seule* machine à faire du chocolat
...
====
Si vous étudiez le code, vous constatez qu'ils ont essayé très soigneusement d'éviter les catastrophes, par exemple :

*   de vider deux mille litres de mélange qui n'a pas bouilli, 
...
|===
|Thread 1 |Thread 2
|b1=new BouilleurChocolat();
|b2=new BouilleurChocolat()
|b1.remplir();
|
|b1.bouillir();
|
...
. Quelques exemples :
- accès unique à une base de données (SAE développement S2)
- accès à un dispositif physique (bouilleur, machine, ...)
- ...
====
endif::prof[]
//----------------------------------------------------- fin Correction -------------------------
...
[source,java]
-----
include::{sourcedir}/Chocolat/src/BouilleurCptChocolat.java[lines=5..20]
-----
====
//----------------------------------------------------- Correction -------------------------
ifdef::prof[]
...
[CAUTION]
====
Pas de return possible dans un constructeur.
Le new est déjà fait, "trop tard" ...
====
endif::prof[]
//----------------------------------------------------- fin Correction -------------------------
...
public class BouilleurChocolat {
	...
	// reste du code de BouilleurChocolat... (vider(), remplir(), ...)
}
-----
+
...
====
. Oui!
. En implémentant une fonction qui s'en charge (méthode static).
. Extrait de la solution
+
[source,java]
-----
include::{sourcedir}/Chocolat/src/BouilleurSafeChocolat.java[lines=5..21]
-----
+
...
[source,java]
-----
BouilleurSafeChocolat bsf ;
bsf = BouilleurSafeChocolat.getInstance();

bsf.remplir();
// ...
BouilleurSafeChocolat bsf2 ;
bsf2 = BouilleurSafeChocolat.getInstance();

System.out.println (bsf == bsf2); // A votre avis ??
-----
====
endif::prof[]
//----------------------------------------------------- fin Correction -------------------------
...
Vos cauchemars continuent ! Un étudiant mal avisé tente des threads pour contrôler le bouilleur ...

// Mais cette fois ils sont en anglais! Vous voyez un
...
|BouilleurChocolat bc = BouilleurChocolat.getInstance(); =>|BouilleurChocolat bc2 = BouilleurChocolat.getInstance(); =>|
|||
|||
|||
|||
...
*`synchronized`* garantit que un suel thread peut exécuter `getInstance()` à la fois. 

Les threads sont mis en attente si besoin pour que la méthode démarrée dans un thread t1 ne peut pas être démarrée dans un autre thread t2 avant sa terminaison dans le thread t1.
...
WARNING: Il existe d'autres implémentations synchronisées du Singleton. cf. link:https://refactoring.guru/design-patterns/singleton/java/example#example-2[Voir ici]
...
- Test sur 2 thread de récupération du singleton et comparaison des références d'objets
====
endif::prof[]
//----------------------------------------------------- fin Correction -------------------------
