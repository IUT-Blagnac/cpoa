...
..  Étudiez la documentation de la méthode `gc()`. A quoi sert-elle ?
..  Étudiez la documentation de la méthode `exit()`. Essayez la méthode `exit()` avec -20 en paramètre.
..	Rajoutez un affichage après le `exit()`, que se passe-t-il ?
+
NOTE: N'oubliez pas de commenter l'appel de `exit()` si vous continuez le `main()` pour lma suite.
...
image::aventure-assoc.png[link="images/aventure.png"]
...
. Veillez à programmer proprement :
* Faites attention au constructeur
* Faites attention à l'attribut privé/static
* Faites attention à la méthode public/static
. Le modèle proposé ci-dessus (<<fig2>>) est basé sur une association. Pourquoi ne devrait-on pas utiliser `getInstance()` dans le cas
d'une composition (`getInstance()` réalisé dans le constructeur du  `Personnage`) ?
...
Nous allons implémenter des classes de _Singleton_ plus "actives", si on peut dire, dont les objets ont des attributs et des méthodes qui modifient l'état de l'objet. (inspiré d’exercices partagés par https://www.djouabri.com/).
...
.Piège à éviter ?
====
*	Ne pas faire simplement 2 objets `Journalisation`, 1 pour chaque de type de log attendue pour le moment dans ce sujet.
**	Car la banque souhaite faire d'autres catégories de journaux (voir plus haut).
**	Vous risquez de programmer uen suite de "`if`" => voir le principe OCP (Open/closed Principle) qui ne serait pas respecté.
====

. *Implémentez cette solution (sur une copie du projet précédent qui est à rendre séparément)*
...
* Dans une première approche, ne prenez pas en compte l'aspect multithread et les problèmes potentiels induits. Si vous avez une solution éprouvée à ce problème : mettez-la en oeuvre ET testez-la, sinon ne prenez pas en compte le multithreading.
...
. *Amélioration et refactoring* : pour ne pas que chaque nom de journal soit écrit "en dur" à chaque ligne de code (entre ", avec risques d'erreurs), et parce que la banque souhaite s'exporter à l'international, on souhaite définir des constantes de classe pour chaque nom de journal. Pour par exemple "Informations", "Erreurs", "Débits", ... :
