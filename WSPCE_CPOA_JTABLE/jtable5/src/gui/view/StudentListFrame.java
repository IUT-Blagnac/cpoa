...
public class StudentListFrame extends JFrame {
	...
	/**
	 * Variante du constructeur avec initialisation de la liste des students à
	 * traiter
	 */
	...
	private void ajoutStudent() {
		...
		// récupération du code de retour (true/false) et des données saisies
		// (stud)
		popupResult = this.getStudentDialog().ouvrirDialogue(stud,
				StudentDialog.ModeEdition.CREATION);

		if (popupResult) {
			// action validée : "stud" contient les données sur le student à
			// ajouter
	...
	private void modifStudent() {
		...
		if (selectedRow >= 0) {
			...
			if (stud != null) {
				...
				// Création ou réutilisation de la boite de dialog
				// "studentDialog"
				// ouverture en mode "ajout"
				// récupération du code de retour (true/false) et des données
				// saisies (stud)
				popupResult = this.getStudentDialog().ouvrirDialogue(stud,
